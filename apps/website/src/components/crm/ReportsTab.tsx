...
export default function ReportsTab({ tenant }: ReportsTabProps): React.ReactElement {
  ...
  const [stats, setStats] = useState({ avgResponseMin: 0, totalLeads: 0, conversionRate: 0 });
    ...
    async function fetchData(): Promise<void> {
      try {
        ...
        if (dashRes.ok) {
          ...
          // Funnel shows how many made it TO each stage or beyond
          const totalNonLost = funnelEntries[funnelEntries.length - 1] || 1;
  ...
  return (
    <div style={{ padding: '24px', overflowY: 'auto', height: '100vh' }}>
      ...
      <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap', marginBottom: '16px' }}>
        <ChartCard title="Pipeline by Stage" flex="2 1 400px">
        ...
        <ChartCard title="Conversion Funnel" flex="1 1 300px">
          <ResponsiveContainer width="100%" height={280}>
            <FunnelChart>
              <Tooltip contentStyle={tooltipStyle} />
              <Funnel dataKey="value" data={funnelData} isAnimationActive>
                <LabelList position="right" fill="#ccc" fontSize={11} dataKey="name" />
      ...
      <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap' }}>
        <ChartCard title="Lead Trend (Last 4 Weeks)" flex="2 1 400px">
          <ResponsiveContainer width="100%" height={280}>
        ...
        <ChartCard title="Lead Sources" flex="1 1 250px">
          <ResponsiveContainer width="100%" height={280}>
            <PieChart>
              <Pie
                ...
                cy="50%"
                outerRadius={90}
                label={((props: Record<string, unknown>) => `${props.name || ''}: ${props.value || 0}`) as any}
              >
...
function KPICard({ label, value, sub, color }: { label: string; value: string; sub?: string; color: string }): React.ReactElement {
  return (
    <div style={{
      ...
      <div style={{ color: '#8888a0', fontSize: '12px', textTransform: 'uppercase', marginBottom: '8px' }}>{label}</div>
