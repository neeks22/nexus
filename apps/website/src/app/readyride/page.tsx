...
function PasswordGate({ onUnlock }: { onUnlock: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('[readyride] Login error:', err instanceof Error ? err.message : 'unknown');
      setError(true);
      setPassword('');
    }
    setLoading(false);
  ...
  useEffect(() => {
    ...
    async function checkSession(): Promise<void> {
      try {
        ...
        console.error('[readyride] Session check error:', err instanceof Error ? err.message : 'unknown');
      }
    }
    checkSession();
  }, []);
...
function InboxContent(): React.ReactElement {
  ...
  const [archivedPhones, setArchivedPhones] = useState<Set<string>>(() => {
    ...
    } catch (err) { console.error('[readyride] Archive restore error:', err instanceof Error ? err.message : 'unknown'); return new Set(); }
  });
  ...
  const archiveConversation = (phone: string, e: React.MouseEvent): void => {
    ...
    setArchivedPhones((prev) => {
      ...
      try { sessionStorage.setItem('archived_readyride', JSON.stringify(Array.from(next))); } catch (err) { console.error('[readyride] Archive save error:', err instanceof Error ? err.message : 'unknown'); }
      return next;
    });
                      ...
                      <div style={{
                        ...
                        <button
                          onClick={async () => {
                            ...
                            } catch (err) { console.error('[readyride] Resume AI error:', err instanceof Error ? err.message : 'unknown'); alert('Failed to resume.'); }
                          }}
                          style={{
