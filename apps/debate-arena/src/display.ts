...
export function showBanner(): void {
  const width = 60;
  const title = 'NEXUS  DEBATE  ARENA';
  const subtitle = 'Multi-Agent Intelligence • Self-Healing • Real-Time';
...
export function showTopic(topic: string, rounds: number): void {
  const label = chalk.bold.hex('#6366f1')('  TOPIC  ');
  const roundLabel = chalk.dim(`${rounds} round${rounds !== 1 ? 's' : ''}`);
  console.log(chalk.hex('#6366f1')('┌─') + label + chalk.hex('#6366f1')('─────────────────────────────────────────────┐'));
  console.log(chalk.hex('#6366f1')('│') + '  ' + chalk.bold.white(topic) + '  ' + chalk.hex('#6366f1')('│'));
  console.log(chalk.hex('#6366f1')('└─────────────────────────────────────────────────────────────┘'));
  console.log(chalk.dim(`  ${rounds} rounds · 5 agents · self-healing enabled`) + '  ' + roundLabel);
  console.log();
}

// ── Round header ──────────────────────────────────

export function showRoundHeader(round: number, totalRounds: number): void {
  const tag = ` ROUND ${round}/${totalRounds} `;
  const dashes = '━'.repeat(Math.max(0, (54 - tag.length) / 2));
  console.log();
  console.log(
...
export function showAgentThinking(agentName: string, icon: string, color: string): Ora {
  const label = applyColor(`${icon}  ${agentName} is thinking…`, color);
  const spinner = ora({
    text: label,
    spinner: 'dots',
    color: 'white',
...
function healthBar(score: number, width: number = 20): string {
  ...
  let color: string;
  if (score >= 0.85) color = '#10b981';      // green
  else if (score >= 0.4) color = '#f59e0b';  // amber
  else if (score >= 0.15) color = '#f97316'; // orange
...
export function showHealingReport(summary: HealingSummary): void {
  ...
  for (const [agentId, health] of Object.entries(summary.agentHealths)) {
    ...
    console.log(
      chalk.bold.hex('#6366f1')('║') +
        line.padEnd(width + 30) + // +30 for ANSI escape codes
        chalk.bold.hex('#6366f1')('║'),
    );
  }
...
export function showDebateComplete(totalMs: number, totalTokens: TokenUsage): void {
  ...
  console.log(chalk.bold.hex('#10b981')('✓ Debate complete'));
  console.log(
